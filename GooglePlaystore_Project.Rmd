...
---
# DATA SET - GOOGLE PLAY STORE APPS (Source : Kaggle)
...
As there are too many outliers, the mod was used for replacing the null values.
...
As the null remaining are too small compared to the dataset. The missing value will be excluded using listwise method.
...
distribution is right-skewed . there are many values in zero, they come from the free apps. Price variable should be analyzed for only app that are paid considering that for free apps will be always zero. As there are app paid and not paid. This variable make sense analyze it only for apps are paid.
...
But first, lets check the threshold to consider a value outlier to analyse the outliers...
...
real price distribution . right-skewed
...
Once the variables are transformed into logaritmic scale, we found a linear reggresion in log log plot.
...
abline(modelo)
```
The variables have linear correlation when both the variables are in logarithmic scale.

# question 5: We are suppose to compute the chi square for category and installs.
...
H1 = The content rating and type are not independent (dependent)
...
Also, as the content rating changes the type variable could change.
